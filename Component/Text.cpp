...
namespace wlEngine {
	...
	Text::Text(Entity* entity) : Component(entity), language(Language::English){
	...
	void Text::loadChinese(const std::wstring& textStr, const int& maxLineWidth, const int& lineSpace, const int& charWidth, const int& charHeight) {
		...
		for (const wchar_t& character : textStr) {
			...
			if (character == L' ' && totalAdvance == 0) continue;
	...
	void Text::setLanguage(const Language& choice) {
		language = choice;
	}
